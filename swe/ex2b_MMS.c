...
PetscErrorCode AddSourceTerm(RDyApp app, Vec F, PetscReal t) {
  ...
  for (PetscInt icell = 0; icell < mesh->num_cells; icell++) {
    if (cells->is_local[icell]) {
      ...
      /*
      PetscReal dhdt = h0/t0 * ( 1+sin(PI*xc/Lx)*sin(PI*yc/Ly) ) * PetscExpScalar(t/t0); 
      PetscReal dhdx = PI*h0/Lx * PetscExpScalar(t/t0) * sin(PI*yc/Ly) * cos(PI*xc/Lx);
      PetscReal dhdy = PI*h0/Ly * PetscExpScalar(t/t0) * sin(PI*xc/Lx) * cos(PI*yc/Ly);

      PetscReal dudt = u0/t0 * ( 1+sin(PI*xc/Lx)*sin(PI*yc/Ly) ) * PetscExpScalar(t/t0);  
      PetscReal dudx = PI*u0/Lx * PetscExpScalar(t/t0) * sin(PI*yc/Ly) * cos(PI*xc/Lx);
      PetscReal dudy = PI*u0/Ly * PetscExpScalar(t/t0) * sin(PI*xc/Lx) * cos(PI*yc/Ly);

      PetscReal dvdt = v0/t0 * ( 1+sin(PI*xc/Lx)*sin(PI*yc/Ly) ) * PetscExpScalar(t/t0);
      PetscReal dvdx = PI*v0/Lx * PetscExpScalar(t/t0) * sin(PI*yc/Ly) * cos(PI*xc/Lx);
      PetscReal dvdy = PI*v0/Ly * PetscExpScalar(t/t0) * sin(PI*xc/Lx) * cos(PI*yc/Ly);

      f_ptr[icell * ndof + 0] += dhdt + u*dhdx + h*dudx + v*dhdy +h*dvdy;
      f_ptr[icell * ndof + 1] += u*dhdt + h*dudt + 2*u*h*dudx + u*u*dhdx + GRAVITY*h*dhdx + u*h*dvdy + v*h*dudy + u*v*dhdy;
      f_ptr[icell * ndof + 2] += v*dhdt + h*dvdt + u*h*dvdx + v*h*dudx + u*v*dhdx + 2*v*h*dvdy + v*v*dhdy + GRAVITY*h*dhdy;
      */
   
      PetscReal a = (1 + sin(xc/Lx*PI)*sin(yc/Ly*PI))*PetscExpScalar(t/t0);
      PetscReal b = cos(xc/Lx*PI)*sin(yc/Ly*PI)*PetscExpScalar(t/t0);
      PetscReal c = sin(xc/Lx*PI)*cos(yc/Ly*PI)*PetscExpScalar(t/t0);
     
      PetscReal S0 = h0/t0*a + 2*PI*u0*h0/Lx*a*b + 2*PI*v0*h0/Ly*a*c;
      PetscReal S1 = 2*u0*h0/t0*a*a + 2*u0*h0*a*a*PI*u0/Lx*b +
                     (u0*u0*a*a + GRAVITY*h0*a)*PI*h0/Lx*b + 3*PI*u0*v0*h0/Ly*a*a*c;
      PetscReal S2 = 2*v0*h0/t0*a*a + 3*PI*u0*v0*h0/Lx*a*a*b +
                     2*v0*h0*a*a*PI*v0/Ly*c + (v0*v0*a*a + GRAVITY*h0*a)*PI*h0/Ly*c;

      f_ptr[icell * ndof + 0] += S0;
      f_ptr[icell * ndof + 1] += S1;
      f_ptr[icell * ndof + 2] += S2;
...
///// @brief update boundary cells
///// @param [inout] app A RDyApp struct
///// @param [inout] X Vec for initial condition
///// @param [in] t Time
///// @return 0 on success, or a non-zero error code on failure
PetscErrorCode UpdateBC(RDyApp app, Vec X, PetscReal t) {
  PetscFunctionBeginUser;
  ...
  for (PetscInt icell = 0; icell < mesh->num_cells_local; icell++) {
    //if (cells->is_local[icell]) {
      ...
      /*
      if (xc == 0.5 || xc == 4.5 || yc == 0.5 || yc == 4.5) {
        PetscReal h  = h0 * ( 1+sin(PI*xc/Lx)*sin(PI*yc/Ly) ) * PetscExpScalar(t/t0);
        PetscReal u  = u0 * ( 1+sin(PI*xc/Lx)*sin(PI*yc/Ly) ) * PetscExpScalar(t/t0);
        PetscReal v  = v0 * ( 1+sin(PI*xc/Lx)*sin(PI*yc/Ly) ) * PetscExpScalar(t/t0);
        x_ptr[icell * ndof + 0] = h;
        x_ptr[icell * ndof + 1] = h*u;
        x_ptr[icell * ndof + 2] = h*v;
        printf("This is boundary cell! xc, yc = %.2f, %.2f\n", xc, yc);
        printf("t, h, u, v = %.2f, %.4f, %.4f, %.4f\n", t, h, u, v);
      }
      */
      ...
      if (nverts == 3) {
        ...
        printf("Revisit cells with 3 vertices");
        exit(0);

      } else if (nverts == 4) {
        PetscInt offset = cells->vertex_offsets[icell];

        PetscInt nverts_BC_south = 0;
        PetscInt nverts_BC_north = 0;
        PetscInt nverts_BC_west = 0;
        PetscInt nverts_BC_east = 0;

        for (PetscInt ii = 0; ii < 4; ii++) {
          PetscInt vtmp = cells->vertex_ids[offset + ii];
          PetscReal xv = vertices->points[vtmp].X[1];
          PetscReal yv = vertices->points[vtmp].X[0];
          if (yv == 0.0 ) {  // vertice at south BC
            nverts_BC_south ++;
          } else if (yv == Ly ) {  // vertice at north BC
            nverts_BC_north ++;  
          }
          if (xv == 0.0 ) {  // vertice at west BC
            nverts_BC_west ++;
          } else if (xv == Lx ) {  // vertice at east BC
            nverts_BC_east ++;
          }
        }

        if (nverts_BC_south > 1 || nverts_BC_north > 1 || nverts_BC_east > 1 || nverts_BC_west > 1) { // update solution at boundary cell
          PetscReal h  = h0 * ( 1+sin(PI*xc/Lx)*sin(PI*yc/Ly) ) * PetscExpScalar(t/t0);
          PetscReal u  = u0 * ( 1+sin(PI*xc/Lx)*sin(PI*yc/Ly) ) * PetscExpScalar(t/t0);
          PetscReal v  = v0 * ( 1+sin(PI*xc/Lx)*sin(PI*yc/Ly) ) * PetscExpScalar(t/t0);
          x_ptr[icell * ndof + 0] = h;
